...
void Chassis_API_Init(void)
{
    ...
    g_ChassisConfig_t.accLimitStatu = AccLimit_None;
    g_ChassisConfig_t.chassisMode   = CHASSIS_MODE_DISNABEL;
 ...
 void  Chassis_API_Coordinate_Lock( float kp, float deadBand)
{
    ...
    if(delta_x <-deadBand || delta_x >deadBand)
        g_ChassisStatus_t.refSpeed_x = -delta_x * kp;
    else
        g_ChassisStatus_t.refSpeed_x = 0.0f;    
   
    if(delta_y <-deadBand || delta_y >deadBand)
        g_ChassisStatus_t.refSpeed_y = -delta_y * kp;
    else
        g_ChassisStatus_t.refSpeed_y = 0.0f;    
}
...
void Chassis_API_setV(float x, float y,float a, uint8_t lock_flag)
{
    ...
    else if(lock_flag == 2)
    {
        ...
        Chassis_API_Angle_Lock(2.0f, 1);
        Chassis_API_Coordinate_Lock(2.0f, 10);
...
void Chassis_Test(void)
{
    ...
    tim_1ms_task:
        Chassis_API_SpeedSoften_Acclimit();

    Chassis_API_setV(0, 0, 0);
    Chassis_API_CalcWheelSpeed();
    Chassis_API_getDstWheelV(*v1, *v2, *v3, *v4);
