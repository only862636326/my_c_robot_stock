...
{   
    ...
    PID_API_set_parameter(& pMoto_t -> pid_Velocity_t, 
                    10.7000f, 1.000f, 8.0000f, //Kp,      Ki,    Kd, 
                    0.0f, 0.8192 * 8000,3000.0f, 16.0f);
...
void Moto_API_PID_Init_C610(pTYPE_Moto_t pMoto_t)
{
    ...
    PID_API_set_parameter(& pMoto_t -> pid_Velocity_t, 
                    5.0f, 0.05f, 0.0f, //Kp,      Ki,    Kd, 
                    0.0f, 0.8192 * 4000,3000.0f, 16.0f);
                    //deadBand, maxOutput, maxIntegral, maxIncrease
...
int16_t Moto_API_Cal_GetCurrentVelocityMode(pTYPE_Moto_t pMoto_t, float dstVelocity)
{
    ...
    if(pMoto_t -> soft_value != 0.0f)
        FlexibelValue_int16(dstVelocity, pMoto_t -> src_rpm, pMoto_t -> soft_value);
#endif
    //dstVelocity = 
...
{
    int dstPosition = g_MotorInfo_t[1].position;
    int dstvelocity = g_MotorInfo_t[1].velocity;
    int16_t I1;
	
    Moto_API_SetMotoInfo(&moto_1, dstvelocity, dstPosition); 
    
    I1 = Moto_API_GetCurrentVelocityPositionMode(&moto_1, g_set_position, 1000);
    absLimit_int16(&I1, 5000);
    if(flag)
        C620_API_SendCurrentVal(0, I1, 0, 0);
    else
